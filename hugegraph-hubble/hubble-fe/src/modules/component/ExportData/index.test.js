...
it('uses keyboard-operable menu actions instead of click-only anchors', () => {
    ...
    const menu = createExportMenu(key => key, exportJson, exportPng);

    expect(menu.items.map(item => item.label)).toEqual([
        'analysis.canvas.export_json',
        'analysis.canvas.export_png',
    ]);
    expect(menu.items.every(item => typeof item.onClick === 'function')).toBe(true);
